...
def make_lmp(**kwargs):
    ...
    with open(lmp_file_path, 'w') as f:
        ...
        "create_box  "+str(species)+" box  bond/types 1 extra/bond/per/atom 100\n",
        ...
        for i in list(r_dictionary.values()) :
            f.write("group g" + str(i) + " type "+str(i))
            f.write("\ngroup agents  union agents g" +str(i))
            f.write("\n")
        ...
        for i in range(0, r.num_of_reactions):
            for k in r.arrange_reaction(i) :
                f.write("fix bond"+str(j)+" all bond/create 10 "+ str(k[0]) +" "+ str(k[1])+ " 1.0 1 prob 0.5 " + str(r_seed))
                j += 1
                r_seed = r_seed + j
                f.write("\n")
